...
@views function visco_elastic_atmowave_2D()
    ...
    dt = 1e-23 #min(min(min(dx_non, dy_non) .* sqrt.(maximum(ρ .* β)), min(dx_non, dy_non).^2.0 ./ ηa_non) .* 4.5,  dx_non ./ c)        # time step size
    ...
    display(fig)
    save("../Plots/Error_pictures/visco_elastic_atmosphere_upwind/smaller_dt/sim1_step0.png", fig)
    ...
    for i = 1:nt 
        ...
        if i % 1 == 0
            ...
            #sur2 = surface!(ax3, xc_vec, yc_vec, P)
            display(fig1)
            save("../Plots/Error_pictures/visco_elastic_atmosphere_upwind/smaller_dt/sim1_step$i.png", fig1)
            
        end
    end
    cp("visco_elastic_atmoshpere_nondim.jl", "../Plots/Error_pictures/visco_elastic_atmosphere_upwind/smaller_dt/visco_elastic_atmoshpere_nondim.jl")
end