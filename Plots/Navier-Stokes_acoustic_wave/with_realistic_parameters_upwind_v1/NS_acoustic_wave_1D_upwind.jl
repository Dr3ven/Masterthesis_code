...
function ac_wave1D_up()
    ...
    for i = 1:nt
        ...
        ρdVxdt .= .-ρ[2:end-1] .* upwind_center(Vx, av_x(Vx), dx)
        ...
        ρVxdVxdx .= .-(1.0 ./ av_x(ρ)) .* (av_x(ρ) .* Vx[2:end-1]) .* upwind(Vx , Vx, dx)
    ...
    save("../Plots/Navier-Stokes_acoustic_wave/with_realistic_parameters_upwind/All_in_one.png", fig)
    display(fig)
end