...
function update_totalstresses(σxx, σyy, σxy, P, ηa_non, εxx, εyy, εxy)
    σxx .= .-P[2:end-1,2:end-1] .+ 2.0 .* ηa_non .* εxx                                                                          # total stress (dani class 5 equation)
    σyy .= .-P[2:end-1,2:end-1] .+ 2.0 .* ηa_non .* εyy                                                                          # total stress
    σxy .=                         2.0 .* ηa_non .* εxy                                                                            # total stress
    return σxx, σyy, σxy
end
...
function conservative2D()
    ...
    p1 = heatmap!(ax, x2dc, y2dc, P_plt, shading=false)
    ...
    @inbounds for it = 1:100
        ...
        while err > err_threshold
            ...
            dtPT                   .= min.(min.(min.(dx_non ./ abs.(av_x(Vx[:, 2:end-1])), dy_non ./ abs.(av_y(Vy[2:end-1, :]))), min(dx_non, dy_non).^2.0 ./ ηa_non), dx_non ./ c_loc) # time step size for pressure and temperature
            ...
            P                       = update_pressure(P, P_non, ρa_non, rho)
