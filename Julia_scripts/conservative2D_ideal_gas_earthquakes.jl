...
function conservative2D_viscous()
    # Physics
    ...
    rho       .= (rho0 .* P) ./ P0 #rho0 ./ P0 .* P             # equation of state for density depending on pressure
    ...
    rho[inpolygon(x2dc,y2dc,Xp2,Yp2) .== 1.0 .&& y2dc.>=locY.+diam./2.0] .= .-0.0 .* (y2dc[inpolygon(x2dc,y2dc,Xp2,Yp2).==1.0 .&& y2dc.>=locY.+diam./2.0].+0.15.*Ly).*drho./(-0.15.-(locY.+diam./2.0)).+rho0    # initial density in the conduit
    P         .= (P0 .* rho) ./ rho0 #P0 ./ rho0 .* rho                                                              # equation of state for pressure depending on density

    @. η += eta_air * (maskrho_air[2:end-1, 2:end-1] == 1.0) + eta_solid * (maskrho_air[2:end-1, 2:end-1] == 1.0)  # initial viscosity distribution    # So sollte der Conduitverlauf sein: lines!(ax, x, .-(100 .+ x .+ (x.^2.0 ./ 2.0) .+ (x.^3.0 ./ 3.0)))
    ...
    for it = 1:350                                                   # 60 iterations max for all equilibrated
        ...
        while err > 1.0e-3
            ...
            @timeit "compute dt" dt = minimum([dx./maximum(abs.(Vx)), dy./maximum(abs.(Vy)), min(dx, dy) .* sqrt(maximum(rho .* beta_vec)), min(dx, dy).^2.0./eta]).*4.0 # time step size
            @timeit "compute c_loc" c_loc .= 1.0./sqrt.(rho[2:end-1,2:end-1].* beta_vec[2:end-1,2:end-1])                                                    # local speed of sound
            @timeit "compute dtPT" dtPT .= min.(min.(min.(dx ./ abs.(av_x(Vx[:, 2:end-1])), dx ./ abs.(av_y(Vy[2:end-1, :]))), min(dx, dy).^2.0 ./ eta .* ones(nx, ny)), dx ./ c_loc) # time step size for pressure and temperature
            ...
            #@timeit "compute P" P              .= ((1.0 ./ beta).* (rho0 ./ rho) .+ P0) .* maskrho_air             # equation of state for pressure depending on density
            @timeit "compute P" P              .= P0 ./ rho0 .* rho #.*maskrho_air                                              # equation of state for pressure depending on density
            ...
            @timeit "compute Sxx" Sxx          .= .-P[2:end-1,2:end-1] .+ 2.0.*eta.*Exx                                                                          # stress in x-direction
            @timeit "compute Syy" Syy          .= .-P[2:end-1,2:end-1] .+ 2.0.*eta.*Eyy                                                                          # stress in y-direction
            @timeit "compute Sxy" Sxy          .=                         2.0.*eta.*Exy                                                                            # stress in xy-direction
            @timeit "compute Szz" Szz          .= .-P[2:end-1,2:end-1] .+ 2.0.*eta.*Ezz                                                                           # stress in z-direction
            @timeit "compute dtV" dtV           = 1.0 ./ (1.0 ./ dt .+ 1.0 ./ (min(dx,dy).^2.0 ./ eta ./ 4.0)) .* CFL_V                                           # time step size for velocity
            ...
            if mod(iter, 25) == 0
                it_counter += 1
                @timeit "print err" print("err = $err\n")
