...
function ac_wave1D()
    ...
    # Grid definition
    xv = 0.0:dx:Lx                          # grid vertice nodes in x-direction
    xc = av_x(xv)                           # grid center nodes in x-direction
    ...
    P .= P0 .+ dP                                               # initial pressure
    c = sqrt(K / ρ0)                                            # speed of sound
    dρ = dP ./ c^2.0                                            # initial density difference
    ρ .+= dρ                                                    # initial density

    E .= P ./ ((γ - 1.0)) + 0.5 .* av_x(Vx).^2                  # initial total energy
    e .= P ./ (γ - 1.0) ./ ρ                                    # initial interal energy
    ...
    #save("Pictures/4_in_one_noncons_acoustic_withenergy.png", fig)
    display(fig)
end