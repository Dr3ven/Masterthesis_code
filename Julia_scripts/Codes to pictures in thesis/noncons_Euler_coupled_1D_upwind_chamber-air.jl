...
function coupled_wave1D_up()
    ...
    # Grid definition
    xc = 0:dx:Lx-dx                         # grid center nodes in x-direction
    xv = 0:dx:Lx                            # grid vertice nodess in x-direction
    ...
    # IC for solid + chamber
    ρ[1:idx_chamber2] .= ρ0 .+ 3.0
    P[1:idx_chamber2] .= P0_c .+ 1.0e5
    ...
    # Analytical solution 
    # Sod shock analytical solution doesnt work for the parameters for this setup! Imaginary numbers pop up!
    # problem = ShockTubeProblem(
    #            geometry = (0.0, Lx, 649.5), # left edge, right edge, initial shock location
    #            left_state = (ρ = 2803.0, u = 0.0, p = P0_c + 1.0e5),
    #            right_state = (ρ = 1.225, u = 0.0, p = P0_a),
    #            t = 0.1, γ = γ)
    ...
    for i = 1:nt
        ...
        # Equation of state for pressure of a isentropic gas
        ...
        if i % divisor == 0
            ...
            # Borders between the three parts in the model
            vlines!(ax1, Lx*(33/100), color=:orange)
            vlines!(ax2, Lx*(33.5/100), color=:orange)
            vlines!(ax3, Lx*(33/100), color=:orange)
            vlines!(ax4, Lx*(33/100), color=:orange)
