...
#function conservative2D_coupled()
    ...
    # So sollte der Conduitverlauf sein: lines!(ax, x, .-(100 .+ x .+ (x.^2.0 ./ 2.0) .+ (x.^3.0 ./ 3.0)))
    ...
    Colorbar(fig[1, 2], p1, label="velocity [m/s]")
    ...
    for it = 1:350                                                   # 60 iterations max for all equilibrated
        ...
        while err > 1.0e-3
            ...
            #@timeit "compute P" P              .= ((1.0 ./ beta).* (rho0 ./ rho) .+ P0) .* maskrho_solid             # equation of state for pressure depending on density
        ...
        if mod(it-1, 1) == 0
            fig1 = Figure()
            ax1 = Axis(fig1[1,1], title="time = $time")
            ...
            stepsize = 10
            arrows!(ax1, X[3:stepsize:end, 1], Y[1, 3:stepsize:end], U[3:stepsize:end, 3:stepsize:end], V[3:stepsize:end, 3:stepsize:end], arrowsize=7, color = :white)
            display(fig1)
        end
    end
    print_timer()
