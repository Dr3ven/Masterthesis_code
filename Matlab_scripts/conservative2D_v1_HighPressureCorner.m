...
nout    = 1;
c       = 1/sqrt(rho0*beta);
dt      = min([dx.^2/D/2.1,dx/abs(Vx0)/2.1,dx/c/2.1])/8;
...
%         FMxy         = av_y(Vy(2:end-1, :     )).*av_x(Mx(:,2:end-1));
        dMxdt       = (Mx-Mx_old)/dt;
...
%         MyRes       = MyRes*0;
        My(2:end-1,2:end-1)  = My(2:end-1,2:end-1) + MyRes*dt;
        Vy           = My./av_y(rho);
        My(1,:)      = -My(2,:);
        My(end,:)    = -My(end-1,:);
        if mod(iter-1, 1) == 0
...
function A = av_xy(B)
    A = 0.25*(B(2:end,2:end)+B(2:end,1:end-1)+ B(1:end-1,2:end)+B(1:end-1,1:end-1));
end
