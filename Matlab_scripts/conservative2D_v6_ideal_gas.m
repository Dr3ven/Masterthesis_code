...
radV      = sqrt((x2dV -locX).^2 + (y2dV -locY).^2);
Xp        = [-1/2 -1/2 -0.3 -1/8 -0.01 -0.01  0.01  0.01  1/8  0.3  1/2  1/2]*Lx;
Yp        = [-1/2 -1/5 -1/5 -0.1 -0.15  -0.28 -0.28 -0.15  -0.1 -1/5 -1/5 -1/2]*Ly;
...
maskV   = 1-inpolygon(x2dV ,y2dV ,Xp,Yp);
maskV(radV <diam/2) = 1;
[x1,y1] = contour(x2dV,y2dV,maskV,[0.5 0.5]);
Xmask   = [ (Lx+dx)/2 (Lx+dx)/2 x1(1,2:end) -(Lx+dx)/2 -(Lx+dx)/2];
Ymask   = [-(Ly+dy)/2 -2000     x1(2,2:end) -2000      -(Ly+dy)/2];
clf
...
rho(inpolygon(x2dc,y2dc,Xp2,Yp2)==1 & y2dc>=locY+diam/2) = -0*(y2dc(inpolygon(x2dc,y2dc,Xp2,Yp2)==1 & y2dc>=locY+diam/2)+0.15*Ly)*drho/(-0.15*Ly-(locY+diam/2))+rho0; 
% rho       = rho.*maskrho;
P         = P0/rho0*rho; 
figure(1), clf
pcolor(x2dc,y2dc,P), shading flat,title('0'), colorbar, hold on
% plot(polyshape(Xp,Yp)), drawnow
...
for it = 1:200
    ...
    while err > 1e-3
        iter = iter+1;
        beta         = 1./P;
        dt           = min([dx/max(abs(Vx(:))),dy/max(abs(Vy(:))),min(dx,dy)*sqrt(max(rho(:).*beta(:))),min(dx,dy).^2/mu])*4;
%         dt           = min([min(dx,dy)*sqrt(max(rho(:))*beta),min(dx,dy).^2/mu]);
        c_loc        = 1./sqrt(rho(2:end-1,2:end-1).*beta(2:end-1,2:end-1));
...
%         rhoRes       = rhoRes.*maskrho(2:end-1,2:end-1);
        rho(2:end-1,2:end-1) = rho(2:end-1,2:end-1) + rhoRes.*dtPT.*CFL_P;
...
%     plot(circ_X,circ_Y,'k',circ_X,-circ_Y,'k')
    X = av_xy(x2dc); Y = av_xy(y2dc); U= av_y(Vx); V=av_x(Vy);
    step = 5;
    %pcolor(X,Y+2e3,sqrt(U.^2+V.^2)), shading flat,  caxis([0 340]), cbar = colorbar,cbar.Title.String = 'v [m/s]', hold on
    imagesc(av_x(Xc')',av_y(Yc)+2e3,sqrt(U.^2+V.^2)'), set(gca,'ydir','normal'), caxis([0 340]), cbar = colorbar,cbar.Title.String = 'v [m/s]', hold on
    xlabel('X [m]'), ylabel('Z [m]'), title(['t = ' num2str(time) ' s']),
    quiver(X(1:step:end,1:step:end),Y(1:step:end,1:step:end)+2e3,U(1:step:end,1:step:end),V(1:step:end,1:step:end),'w')
    patch(Xmask,Ymask+2e3,[0.7 0.7 0.7],'edgecolor','none'), hold off, drawnow
    print(['Slow_Eruption_it' num2str(it)],'-dpng')
    save('BreakPoint.mat')
    end
end
